...
Vue.component('aemadwithapdate',
    {
        template: `
    
<div>
<h1 style="margin-right: 3%;">فريد <span class="material-icons" style="    font-size: 60px;
    color: #e4cb96;">
    pending_actions
    </span></h1>
...
`,
        ...
        methods:
        {
            ...
            post() {
                ...
                this.$http.delete(`https://farid-8c572-default-rtdb.firebaseio.com/appointments.json`)
            },
            update(event) {
            ...
            removed(event) {
                ...
                firebase.database().ref('appointments/' + name).child(key).remove()
                    .then(function () {
                        alert("تمت عمليت الغاء الحجز بنجاح الان ننتقل لبعث تنبية للعميل")
                        reload();
                                ...
                                if ((parseInt(data[key].time[0] + data[key].time[1]) == timeh)) {
                                    ...
                                    if (parseInt(data[key].time[3] + data[key].time[4]) <= timem) {
                                       // firebase.database().ref('appointments/').child(key).remove()
                                                ...
                                                firebase.database().ref('appointments/' + savedDate[x].customerName).child(key).update(
                                                    {
                                                        status: 'updateted'
                                                    }, function (error) {
                                                        if (!error) {
                                    ...
                                    console.log(parseInt(data[key].time[0] + data[key].time[1]))
                                    console.log(timeh)
                                    //firebase.database().ref('appointments/').child(key).remove()
